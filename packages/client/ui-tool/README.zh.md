...
工具调用在对话中显示为卡片：一个根调用树带其嵌套子调用，每个原子调用由所属视图渲染。用户看到运行中、成功、失败与中断状态，这些状态只来自冻结的调用/结果切片。展开的正文带有一个悬停显现的复制 pill，会把 `serializeToolCall` 对该调用的 JSON 投影写入宿主剪贴板。用户可通过宿主回调打开文件或检查调用。
